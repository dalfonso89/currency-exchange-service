...
// TestConcurrentRatesRequestsWithMocks tests the rates endpoint with high concurrent load using mocks
func TestConcurrentRatesRequestsWithMocks(t *testing.T) {
	// Create mock servers
	...
	// Log test results
	t.Logf("=== Concurrent Load Test Results (with Mocks) ===")
	...
	if errorCount > 0 {
		t.Errorf("Too many errors: %d (expected 0)", errorCount)
	}
}

// TestRaceConditionDetectionWithMocks tests for race conditions using mocks
func TestRaceConditionDetectionWithMocks(t *testing.T) {
	// Create mock servers
	mockExchangeRateServer := testutils.NewMockExchangeRateServer()
	mockJSONPlaceholderServer := testutils.NewMockJSONPlaceholderServer()
	defer mockExchangeRateServer.Close()
...
// TestCacheConsistencyWithMocks tests cache behavior under concurrent load using mocks
func TestCacheConsistencyWithMocks(t *testing.T) {
	// Create mock servers
	mockExchangeRateServer := testutils.NewMockExchangeRateServer()
	mockJSONPlaceholderServer := testutils.NewMockJSONPlaceholderServer()
	defer mockExchangeRateServer.Close()
...
// TestStressLoadWithMocks tests the service under extreme load using mocks
func TestStressLoadWithMocks(t *testing.T) {
	// Create mock servers
	mockExchangeRateServer := testutils.NewMockExchangeRateServer()
	mockJSONPlaceholderServer := testutils.NewMockJSONPlaceholderServer()
	defer mockExchangeRateServer.Close()
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(goroutineID int) {
			...
			for j := 0; j < requestsPerGoroutine; j++ {
				...
				req, err := http.NewRequestWithContext(ctx, "GET", server.URL+"/api/v1/rates", nil)
				if err != nil {
	...
	if totalRequests > 0 {
		...
		t.Logf("=== Stress Test Results (with Mocks) ===")
		t.Logf("Total Requests: %d", totalRequests)
		t.Logf("Successful Requests: %d (%.2f%%)", successfulRequests, successRate)
		t.Logf("Failed Requests: %d", failedRequests)
		t.Logf("Errors: %d", errorCount)
		...
		if successRate < 90.0 {
			t.Errorf("Stress test success rate too low: %.2f%% (expected >= 90%%)", successRate)
		}

		if avgResponseTime > 2*time.Second {
			t.Errorf("Stress test average response time too high: %v (expected < 2s)", avgResponseTime)
		}
	}
}
