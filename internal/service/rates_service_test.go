...
func TestRatesService_Convert_Success(t *testing.T) {
	cfg := testutils.MockConfig()
	logger := testutils.MockLogger()
	...
	ctx := context.Background()
	result, err := service.Convert(ctx, "USD", "EUR", 100.0)
	if err != nil {
		t.Fatalf("Convert() error = %v", err)
	}

	if result.From != "USD" {
		t.Errorf("Convert() From = %v, want %v", result.From, "USD")
	}
	if result.To != "EUR" {
		t.Errorf("Convert() To = %v, want %v", result.To, "EUR")
	}
	if result.Amount != 100.0 {
		t.Errorf("Convert() Amount = %v, want %v", result.Amount, 100.0)
	}
	if result.Rate != 0.85 {
		t.Errorf("Convert() Rate = %v, want %v", result.Rate, 0.85)
	}
	if result.Converted != 85.0 {
		t.Errorf("Convert() Converted = %v, want %v", result.Converted, 85.0)
	}
	if result.Provider != "test-provider" {
		t.Errorf("Convert() Provider = %v, want %v", result.Provider, "test-provider")
	}
}

func TestRatesService_Convert_RateNotFound(t *testing.T) {
	cfg := testutils.MockConfig()
	logger := testutils.MockLogger()

	// Create a mock provider with limited rates
	mockProvider := &MockProvider{
		name:     "test-provider",
		enabled:  true,
		priority: 1,
	...
	ctx := context.Background()
	_, err := service.Convert(ctx, "USD", "GBP", 100.0)
	if err == nil {
